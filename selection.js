...
function processSelection(e) {
  ...
  if ($.isNumeric(text) && /^(\d{10,13})$/.test(text)) {
...
function convertTimestamp(ts) {
  var multiplier = Math.pow(10, 13 - ts.length);
  var date = new Date(ts * multiplier);
  var dateStr = "";
  ...
  dateStr += (h<=9?'0'+h:h) + ":" + (mi<=9?'0'+mi:mi) + ":" + (s<=9?'0'+s:s) + "." + millis;
